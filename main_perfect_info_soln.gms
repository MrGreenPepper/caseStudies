...
savedEnergy
buildCapacities(scenario,plants, country)
...
***     powerGeneration constraints
generation_constraint_Plants(scenario,t, plants, country)..                     powerGeneration(scenario, t, plants, country) =l= (Plants_capacities(plants, country)+ buildCapacities(scenario, plants, country)) * Plants_availability(plants);
generation_constraint_rePlants(scenario, t, renewablePlants, country)..         powerGeneration(scenario,t, renewablePlants, country) =l= (Plants_capacities(renewablePlants, country) + buildCapacities(scenario, renewablePlants, country)) * CapacityFactor(t, renewablePlants);
...
***     baseload plants ramping up/down 
ramping_upper_constraint(scenario,t, baseLoadPlants, country)..                 powerGeneration(scenario, t, baseLoadPlants, country) =l= powerGeneration(scenario,t-1, baseLoadPlants, country) + ((Plants_capacities(baseLoadPlants, country) + buildCapacities(scenario, baseLoadPlants, country)) * (0.18));
ramping_bottom_constraint(scenario,t, baseLoadPlants, country)..                powerGeneration(scenario, t, baseLoadPlants, country) =g= powerGeneration(scenario,t-1, baseLoadPlants, country) - ((Plants_capacities(baseLoadPlants, country) + buildCapacities(scenario, baseLoadPlants, country)) * (0.18));
...
storage_levelConstraint_maxCapacity(scenario, t, storages, country)..                     storageLevel(scenario, t, storages, country) =l= StorageCapacity(storages, country) + buildCapacities(scenario, storages, country);
storage_loadingConstraint_performance(scenario, t, storages, country)..                   storageLoading(scenario, t, storages, country) =l= (StorageCapacity(storages, country) + buildCapacities(scenario, storages, country)) / 6;
...
***     investment costs
capacityCosts(country)..                                                        totalInvestmentCosts =e= sum((plants, scenario), ScenarioData(scenario, 'probability')*buildCapacities(scenario, plants, country) * InvestmentCosts(plants));


***     water power constraint
waterConstraint(country)..                                                      sum((constraintInvestsPlants), Plants_capacities(constraintInvestsPlants, country)) * WaterCapacitieFactors(country) =g= sum((constraintInvestsPlants), Plants_capacities(constraintInvestsPlants, country) + sum(scenario, ScenarioData(scenario, 'probability')*buildCapacities(scenario, constraintInvestsPlants, country)));
...
*StageOneInvestmentsDecision(plants, country) = buildCapacities.l(scenario, plants, country);
...
display buildcapacities.l,  totalInvestmentCosts.l
